...
from parsifal.apps.library.forms import DocumentForm, FolderForm, SharedFolderForm
from parsifal.apps.library.models import Document, Folder, SharedFolder
...
def list_actions(request):
    ...
    if action == "remove_from_folder":
        ...
        return copy(request)
    redirect_to = request.POST.get("redirect", reverse("library:index"))
...
def new_document(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            json_context["status"] = "success"
            json_context["redirect_to"] = reverse("library:index")
        else:
            json_context["status"] = "error"
    else:
        form = DocumentForm()
...
def move(request):
    ...
    messages.success(
        request,
        "Documents moved from folder {0} to {1} successfully!".format(move_from_folder.name, move_to_folder.name),
    )
    redirect_to = request.POST.get("redirect", reverse("library:index"))
    return redirect(redirect_to)
...
def copy(request):
    ...
    try:
        ...
        messages.error(request, _("The folder you are trying to copy does not exist."))
        return redirect(redirect_to)
    ...
    messages.success(request, "Documents copied to folder {0} successfully!".format(copy_to_folder.name))
    return redirect(redirect_to)
...
def remove_from_folder(request):
    ...
    messages.success(
        request,
        "{0} {1} successfully removed from folder {2}!".format(
            documents_size, get_document_verbose_name(documents_size), folder.name
        ),
    )
    redirect_to = request.POST.get("redirect", reverse("library:index"))
    return redirect(redirect_to)
...
def delete_documents(request):
    ...
    documents.delete()
    messages.success(
        request, "{0} {1} successfully deleted!".format(documents_size, get_document_verbose_name(documents_size))
    )
    redirect_to = request.POST.get("redirect", reverse("library:index"))
    return redirect(redirect_to)
        ...
        if any(documents):
            ...
            if folder_id:
                try:
                    ...
                    messages.error(request, "Folder does not exists.")
            messages.success(request, "{0} documents imported!".format(len(documents)))
        else:
